                    ...
                    <?php foreach($listTiket as $tiket): ?>
                        <tr class="hover:bg-slate-700 transition">
                            ...
                            <td class="px-4 py-2 text-slate-300 text-center">
                                ...
                                // Buat image baru lebih tinggi
                                $barcodeImg = imagecreatefromstring($barcodeImage);
                                $width = imagesx($barcodeImg);
                                $height = imagesy($barcodeImg);

                                $newHeight = $height + 20; // ruang untuk angka
                                $newImg = imagecreatetruecolor($width, $newHeight);

                                // Warna background
                                $bg = imagecolorallocate($newImg, 30, 41, 59); // sama kaya bg-slate-800
                                $txtColor = imagecolorallocate($newImg, 203, 213, 225); // slate-300
                                imagefill($newImg, 0, 0, $bg);

                                // Copy barcode ke image baru
                                imagecopy($newImg, $barcodeImg, 0, 0, 0, 0, $width, $height);

                                // Tulis angka di bawah
                                $font = __DIR__ . '/../../fonts/RobotoMono-Italic-VariableFont_wght.ttf'; // pastikan file ada
                                $xPos = intval(($width - 7.2 * strlen($barcodeValue)) / 2);
                                $yPos = $height + 15;$fontSize = 12;
$spacing = 10; // jarak antar angka, bisa disesuaikan
$y = $height + 15; // posisi vertikal angka

// Hitung total panjang teks agar bisa ditengah
$totalWidth = (strlen($barcodeValue) * $spacing);
$startX = ($width - $totalWidth) / 2;

for ($i = 0; $i < strlen($barcodeValue); $i++) {
    $char = $barcodeValue[$i];
    $x = $startX + ($i * $spacing);

    imagettftext($newImg, $fontSize, 0, $x, $y, $txtColor, $font, $char);
}
                                ...
                                imagedestroy($newImg);
                                ?>
                                <img src="data:image/png;base64,<?= base64_encode($data) ?>" class="h-auto w-40 mx-auto">
                            </td>
